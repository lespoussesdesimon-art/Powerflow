...
const { 
  Play, Pause, RotateCcw, CheckCircle, Calendar, User, Dumbbell, Heart, 
  Flower2, ChevronDown, ChevronUp, Plus, Edit, Trash2, Settings, ArrowLeft, 
  Save, X, Clock, Coffee, Brain, Target, TrendingUp, Award, Menu
} = lucide;
...
const getInitialData = () => ({
  ...
  bodyCycle: {
    ...
    active: true,
    currentPosition: 0,
    lastCompletedDate: null, // Date de la dernière séance complétée (format ISO)
...
function PowerflowApp() {
  ...
  const [mindExerciseFormData, setMindExerciseFormData] = useState({
    name: '',
    type: 'chrono', // 'chrono' = chronomètre (0tox), 'reps' = répétitions
    target: '' // Objectif : nombre de reps OU durée en secondes
  });
  ...
  // Fonction pour obtenir la date actuelle (réelle ou simulée)
  ...
  const getLastPerformance = (workoutId, exerciseId, exerciseIndex) => {
    // Chercher la dernière séance de ce workout dans l'historique
  ...
  const getLastHabitPerformance = (habitId) => {
    // Chercher la dernière séance de cette habitude dans l'historique
  ...
  const isWorkoutScheduledToday = (workout) => {
    ...
    // Si déjà complété aujourd'hui, on l'affiche quand même mais grisé
    // (la logique de grisage est gérée dans le render)
    
    const today = getCurrentDate();
    const dayOfWeek = today.getDay(); // 0 = dimanche, 1 = lundi, etc.
    
    // Type: Cycle - géré séparément dans le cycle
    if (workout.frequency.type === 'cycle') {
      return false; // Les workouts du cycle sont affichés via le système de cycle
    }
    
    // Type: Jours de la semaine
    if (workout.frequency.type === 'weekdays') {
      // Toujours afficher si c'est un jour programmé (même si complété aujourd'hui)
      return workout.frequency.weekdays?.includes(dayOfWeek) || false;
    }
    
    // Type: Récurrence (tous les X jours)
    if (workout.frequency.type === 'interval') {
      ...
      // Afficher si c'est le jour prévu OU si déjà fait aujourd'hui (pour montrer grisé)
      // daysSinceLastCompleted === 0 signifie "fait aujourd'hui" → on affiche quand même
  ...
  const handleCycleDragOver = (e, index) => {
    ...
    setData(prev => ({
      ...
      bodyCycle: {
        ...
        // Réinitialiser lastCompletedDate pour que le nouvel ordre s'affiche immédiatement
  ...
  const deleteExercise = (id) => {
    if (!confirm('Supprimer cet exercice ? Il sera retiré de tous les entraînements.')) return;
  ...
  const saveMindExercise = () => {
    ...
    if (editMindExercise) {
      setData(prev => {
        ...
        // Mettre à jour toutes les habitudes qui utilisent cet exercice
  ...
  const createHabitFromExercise = (exerciseId) => {
    ...
    const newHabit = {
      ...
      lastCompletedDate: null, // Date de dernière complétion
      progression: {
        enabled: false,
        type: 'weekly',
        change: 300
    ...
    alert(`Habitude "${exercise.name}" créée !`);
  };
  ...
  const createNewWorkout = () => {
    const newWorkout = {
      ...
      frequency: {
        ...
        lastCompleted: null // Date de dernière complétion pour calculer la récurrence
      }
    };
  ...
  const startWorkout = (workoutId) => {
    ...
    if (!workout || workout.exercises.length === 0) {
      alert('Cet entraînement ne contient aucun exercice');
      return;
    }
  ...
  const completeExercise = () => {
    ...
    // Save exercise data - utiliser exerciseId + index pour supporter les exercices répétés
    ...
    if (currentExerciseIndex === workout.exercises.length - 1) {
      ...
      setTimer(restTime);
      setIsTimerRunning(true); // Démarrer automatiquement le repos
      setIsInRestPeriod(true); // Marquer qu'on est en période de repos
  ...
  const completeWorkout = (finalData) => {
    ...
    setData(prev => {
      const newData = {
        ...
        // Mettre à jour lastCompleted pour le workout concerné
        workouts: prev.workouts.map(w =>
      ...
      // Si c'est un workout du cycle, mettre à jour bodyCycle.lastCompletedDate
  ...
  useEffect(() => {
    if (activeWorkout && !isInRestPeriod) {
      ...
      if (exercise?.type === 'timing' && exercise.duration) {
        ...
        setIsTimerRunning(true); // Démarrer automatiquement les exercices timing
      } else if (exercise?.type === 'chrono') {
        setTimer(0);
        setIsTimerRunning(false); // Les chrono ne démarrent pas automatiquement
  ...
  useEffect(() => {
    ...
    if (isTimerRunning && activeWorkout) {
      ...
      interval = setInterval(() => {
        setTimer(prev => {
          // Si on est en repos, toujours décrémenter (compte à rebours)
          if (isInRestPeriod) {
            if (prev <= 1) {
              setIsTimerRunning(false);
              setIsInRestPeriod(false);
          ...
          // Pour les exercices de type chrono, on incrémente (compte vers le haut)
          if (exercise?.type === 'chrono') {
            return prev + 1;
          }
          
          // Pour les autres types d'exercices (timing, reps), on décrémente
          if (prev <= 1) {
            setIsTimerRunning(false);
            return 0;
          }
  ...
  useEffect(() => {
    // Ne pas auto-avancer en mode debug pour éviter de modifier les vraies données
    if (debugMode) return;
    
    const checkAndAdvanceCycle = () => {
      const lastCompleted = data.bodyCycle.lastCompletedDate;
      if (!lastCompleted) return; // Pas encore de séance complétée
      ...
      // Si la dernière séance était hier ou avant, avancer le cycle
  ...
  const handleRestDay = (take) => {
    if (take) {
      // Take the rest - enregistrer comme complété
      setData(prev => ({
        ...prev,
        bodyCycle: {
          ...prev.bodyCycle,
      ...
      <div className="space-y-6">
        ...
        <p className="text-gray-400">
          Aujourd'hui, {getCurrentDate().toLocaleDateString('fr-FR', { 
            ...
            month: 'long', 
            year: 'numeric' 
          })} • {getCurrentDate().toLocaleTimeString('fr-FR', {
            hour: '2-digit',
            minute: '2-digit'
          })}
        </p>
        ...
        {debugMode && (
          <div className="bg-yellow-500/20 border-2 border-yellow-500 rounded-xl p-4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-bold text-yellow-300">🧪 Mode Debug - Simulation du temps</h3>
              <button
                onClick={() => {
                  setDebugMode(false);
            ...
            <div className="bg-yellow-500/10 rounded p-2 mb-3 text-xs">
              <p className="text-yellow-200">
                ⚠️ <strong>Mode lecture seule :</strong> L'auto-avancement du cycle est désactivé en mode debug.
                Tu peux voir comment l'app s'affichera à cette date, mais les données ne sont pas modifiées automatiquement.
              </p>
            </div>
        ...
        {activeWorkout && (
          <div className="bg-orange-500/20 border-2 border-orange-500 rounded-xl p-6">
            <h2 className="text-xl font-bold mb-2">⏸️ Entraînement en pause</h2>
        ...
        <div className="bg-white/10 rounded-xl p-6">
          <h2 className="text-xl font-bold mb-4">💪 Corps</h2>
          ...
          {currentStep && currentStep.type === 'workout' && (
            <div className="mb-4">
              {(() => {
                ...
                if (!workout) return <p>Entraînement introuvable</p>;
                const IconComp = getIconComponent(workout.icon);
                
                if (isBodyCompletedToday) {
                  // Séance terminée aujourd'hui - affichage grisé
                  return (
                    <div className="bg-white/5 rounded-lg p-4 opacity-60">
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs opacity-75 mt-1">🔄 Cycle</p>
                          <p className="text-sm text-green-400 mt-1">✓ Fait aujourd'hui</p>
                        </div>
                      </div>
                    </div>
                  );
                ...
                // Séance non terminée - affichage normal avec bouton
                return (
                  <div>
                    <div className={`${workout.color} rounded-lg p-4`}>
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <div className="flex-1">
                          <h3 className="text-xl font-bold">{workout.name}</h3>
                          <p className="text-xs opacity-75">🔄 Cycle</p>
                        </div>
                      </div>
                      <p className="text-sm opacity-75 mb-4">
                      ...
                      {activeWorkout && activeWorkout.id === workout.id && (
                        <div className="bg-orange-500/20 rounded-lg p-3 text-center">
                          <p className="text-sm text-orange-300">⏸️ En pause</p>
                        </div>
                      )}
                      {activeWorkout && activeWorkout.id !== workout.id && (
                        <div className="bg-white/5 rounded-lg p-3 text-center">
                          <p className="text-sm opacity-50">Un autre entraînement est en cours</p>
                        </div>
                      )}
                    </div>
                    {daysLate > 0 && (
                      <div className="mt-2 text-center">
                        <p className="text-sm text-orange-400">⚠️ {daysLate} jour{daysLate > 1 ? 's' : ''} de retard</p>
                      </div>
                    )}
                  </div>
                );
          ...
          {/* Scheduled Workouts - Récurrence et Jours de la semaine */}
          {(() => {
            const scheduledWorkouts = data.workouts.filter(w => isWorkoutScheduledToday(w));
            
            if (scheduledWorkouts.length === 0 && !currentStep) {
              return <p className="text-sm opacity-75">Aucun entraînement aujourd'hui</p>;
            }
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-xs opacity-75 mt-1">
                              {workout.frequency.type === 'interval' && `⏱️ Tous les ${workout.frequency.interval} jour${workout.frequency.interval > 1 ? 's' : ''}`}
                              {workout.frequency.type === 'weekdays' && `📆 Jours de la semaine`}
                            </p>
                            <p className="text-sm text-green-400 mt-1">✓ Fait aujourd'hui</p>
                          </div>
                        </div>
                    ...
                    <div key={workout.id} className={`${workout.color} rounded-lg p-4`}>
                      ...
                      {activeWorkout && activeWorkout.id === workout.id && (
                        <div className="bg-orange-500/20 rounded-lg p-2 text-center">
                          <p className="text-xs text-orange-300">⏸️ En pause</p>
                        </div>
                      )}
                      {activeWorkout && activeWorkout.id !== workout.id && (
                        <div className="bg-white/5 rounded-lg p-2 text-center">
                          <p className="text-xs opacity-50">Un autre entraînement est en cours</p>
                        </div>
                      )}
                    </div>
                  );
          ...
          <div className="bg-white/10 rounded-xl p-6">
            ...
            {(() => {
              if (isBodyCompletedToday) {
                ...
                return (
                  <div className="bg-white/5 rounded-lg p-4 opacity-60">
                    <h3 className="text-xl font-bold mb-2">☕ {currentStep.name}</h3>
                    <p className="text-sm text-green-400">✓ Fait aujourd'hui</p>
                  </div>
                );
              }
              ...
              if (currentStep.optional) {
                return (
                  <div>
                    <div className="bg-green-500 rounded-lg p-4">
                      <h3 className="text-xl font-bold mb-3">☕ {currentStep.name} (Optionnel)</h3>
                      <p className="text-sm opacity-75 mb-4">
                ...
                return (
                  <div>
                    <div className="bg-green-500 rounded-lg p-4">
                      <h3 className="text-xl font-bold mb-3">☕ {currentStep.name}</h3>
                      <p className="text-sm opacity-75 mb-4">
                        Jour de repos obligatoire pour récupérer
                      </p>
                      <button
                        onClick={() => handleRestDay(true)}
                        className="w-full bg-white/20 hover:bg-white/30 rounded-lg p-3 font-bold transition-colors"
        ...
        {/* Today's Task - Mind */}
        <div className="bg-white/10 rounded-xl p-6">
          <h2 className="text-xl font-bold mb-4">🧠 Esprit</h2>
          {!data.mindHabits || data.mindHabits.length === 0 ? (
            <p className="text-sm opacity-75">Aucune habitude créée</p>
          ) : (
            <div className="space-y-3">
              {data.mindHabits.map(habit => {
                ...
                if (habitCompletedToday) {
                  // Habitude terminée aujourd'hui - affichage grisé
                  return (
                ...
                // Habitude non terminée - affichage normal avec bouton
                return (
                  <div key={habit.id} className={`${habit.color} rounded-lg p-4`}>
                    <h3 className="font-bold mb-2">{habit.name}</h3>
                    <p className="text-sm opacity-75 mb-3">
                      {habit.type === 'reps' ? '🔢 Répétitions' : '⏱️ Chronomètre'}
                    </p>
                    {(() => {
                      ...
                      if (habitDaysLate > 0) {
                        return (
                          <p className="text-xs text-orange-400 mb-3">
                            ⚠️ {habitDaysLate} jour{habitDaysLate > 1 ? 's' : ''} de retard
                          </p>
                        );
                      }
                      return null;
                    ...
                    <button
                      onClick={() => {
                        ...
                        setHabitTimer(0); // Toujours commencer à 0
                        setIsHabitTimerRunning(false);
                        setRepsInput(''); // Réinitialiser l'input de reps
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <button 
            onClick={() => {
              // Mettre en pause - juste revenir à l'accueil sans arrêter le timer
              // Le timer continuera à tourner en arrière-plan
              setCurrentView('dashboard');
            }}
          ...
          <div className="flex gap-2">
            ...
            <button 
              ...
              title="Abandonner l'entraînement"
            >
              <X size={24} />
            </button>
          </div>
        ...
        <div className="bg-white/10 rounded-xl p-6">
          {isInRestPeriod ? (
            <div className="text-center">
              <h2 className="text-2xl font-bold mb-4">⏸️ Repos</h2>
              ...
              <button
                onClick={() => {
                  setTimer(0);
                  setIsTimerRunning(false);
                  setIsInRestPeriod(false); // Sortir de la période de repos
                }}
            ...
            <div>
              ...
              {exercise.type === 'reps' && (
                <div className="mb-4">
                  <div className="bg-white/5 rounded-lg p-3 mb-3 space-y-1">
                    <p className="text-sm opacity-75">🎯 Objectif : <span className="font-bold">{exercise.target}</span></p>
                    {(() => {
                      const lastPerf = getLastPerformance(activeWorkout.id, exerciseId, currentExerciseIndex);
                      if (lastPerf && lastPerf.reps) {
                        return (
                          <p className="text-sm opacity-75">📊 Dernière fois : <span className="font-bold text-blue-400">{lastPerf.reps} reps</span></p>
                        );
                      }
                      return null;
                    })()}
              ...
              {exercise.type === 'chrono' && (
                <div className="mb-4 text-center">
                  <div className="bg-white/5 rounded-lg p-3 mb-3">
                    <p className="text-sm opacity-75">🎯 Chronomètre libre</p>
                    {(() => {
                      const lastPerf = getLastPerformance(activeWorkout.id, exerciseId, currentExerciseIndex);
                      if (lastPerf && lastPerf.duration) {
                        return (
                          <p className="text-sm opacity-75 mt-1">📊 Dernière fois : <span className="font-bold text-blue-400">{formatTime(lastPerf.duration)}</span></p>
                        );
                      }
                      return null;
                    })()}
              ...
              {exercise.type === 'timing' && (
                <div className="mb-4 text-center">
                  <div className="bg-white/5 rounded-lg p-3 mb-3">
                    <p className="text-sm opacity-75">🎯 Objectif : <span className="font-bold">{exercise.duration ? formatTime(exercise.duration) : exercise.target}</span></p>
                    {(() => {
                      const lastPerf = getLastPerformance(activeWorkout.id, exerciseId, currentExerciseIndex);
                      if (lastPerf && lastPerf.duration) {
                        return (
  ...
  const renderHabitExecution = () => {
    ...
    // L'habitude est créée depuis un exercice, mais elle n'a pas de lien direct
    // On va stocker le target directement dans l'habitude lors de la création
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <button 
            onClick={() => {
              if (window.confirm('Arrêter cette séance ?')) {
                setCurrentView('dashboard');
                setActiveHabit(null);
              }
            }}
        ...
        <div className={`${activeHabit.color} rounded-xl p-8`}>
          <div className="text-center">
            {isRepsType ? (
              // TYPE: RÉPÉTITIONS
              <div>
                <p className="text-sm opacity-75 mb-2">🔢 Nombre de répétitions</p>
                {target && (
                  <p className="text-sm text-gray-400 mb-2">🎯 Objectif : {target} reps</p>
                )}
                {lastPerformance && lastPerformance.reps && (
                  <p className="text-sm text-blue-400 mb-4">📊 Dernière fois : {lastPerformance.reps} reps</p>
                )}
                <input
                  type="number"
                  value={repsInput}
              ...
              <div>
                {target && (
                  <p className="text-sm text-gray-400 mb-2">🎯 Objectif : {formatTime(parseInt(target) * 60)}</p>
                )}
                {lastPerformance && lastPerformance.duration && (
                  <p className="text-sm text-blue-400 mb-4">📊 Dernière fois : {formatTime(lastPerformance.duration)}</p>
                )}
      ...
      <div className="space-y-6">
        ...
        {data.history.length === 0 ? (
          <div className="bg-white/10 rounded-xl p-8 text-center">
            <p className="opacity-75">Aucune séance enregistrée</p>
          </div>
        ) : (
          <div className="space-y-3">
              ...
              <div key={entry.id} className="bg-white/10 rounded-xl p-4">
                <div className="flex justify-between items-start mb-2">
                  <div className="flex-1">
                    ...
                    <p className="text-sm opacity-75">{entry.type === 'body' ? '💪 Corps' : '🧠 Esprit'}</p>
                  </div>
                  <div className="flex items-center gap-3">
                ...
                {entry.duration && <p className="text-sm">⏱️ {formatTime(entry.duration)}</p>}
                {entry.reps && <p className="text-sm">🔢 {entry.reps} répétitions</p>}
                
                {/* Détails des exercices pour les workouts */}
                {entry.type === 'body' && entry.exercises && (
                  <details className="mt-3">
                    <summary className="text-sm opacity-75 cursor-pointer hover:opacity-100">
                      📋 Voir les détails ({Object.keys(entry.exercises).length} exercices)
                    </summary>
                    <div className="mt-2 space-y-2 pl-4 border-l-2 border-white/20">
                      {Object.keys(entry.exercises).length === 0 && (
                        <p className="text-sm text-red-400">Aucun exercice enregistré</p>
                      )}
                      {Object.entries(entry.exercises)
                        .sort(([keyA], [keyB]) => {
                          // Trier par index (la partie après le -)
                          const indexA = parseInt(keyA.split('-').pop());
                          const indexB = parseInt(keyB.split('-').pop());
                          return indexA - indexB;
                        })
                        .map(([uniqueKey, exerciseData]) => {
                          // La clé est "exerciseId-index", extraire l'exerciceId
                          const exerciseId = uniqueKey.split('-').slice(0, -1).join('-');
                          const exercise = getExerciseById(exerciseId);
                          
                          // Debug : afficher même si exercice non trouvé
                          if (!exercise) {
                            return (
                              <div key={uniqueKey} className="text-sm text-orange-400">
                                <p>⚠️ Exercice introuvable (ID: {exerciseId})</p>
                                <p className="text-xs opacity-75">
                                  {exerciseData.reps && `${exerciseData.reps} reps`}
                                  {exerciseData.duration && ` • ${formatTime(exerciseData.duration)}`}
                                </p>
                              </div>
                            );
                          }
                          ...
                          return (
                            <div key={uniqueKey} className="text-sm">
                              ...
                              {exerciseData.reps && <p className="opacity-75">• {exerciseData.reps} reps</p>}
                              {exerciseData.duration && <p className="opacity-75">• {formatTime(exerciseData.duration)}</p>}
                            </div>
                          );
                        })}
                    </div>
  ...
  const renderSettings = () => {
    return (
      <div className="space-y-6">
        ...
        {/* BODY SECTION */}
        <div className="bg-white/10 rounded-xl p-4">
          <h3 className="font-bold mb-4 text-lg">💪 Corps</h3>
          
          {/* Cycle */}
          <div className="mb-4">
            <p className="text-sm opacity-75 mb-3">Cycle d'entraînement • Glisse pour réorganiser</p>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        {isWorkout ? (
                          ...
                          <span>☕ {step.name} {step.optional && '(optionnel)'}</span>
                        )}
                      </div>
                      {idx === data.bodyCycle.currentPosition && (
          ...
          <div className="space-y-2">
            ...
            >
              <Plus size={20} />
              Ajouter un entraînement au cycle
            </button>
            <button
          ...
          <div className="mt-6 pt-4 border-t border-white/20">
            <h4 className="font-bold mb-3">Mes entraînements</h4>
            {!data.workouts || data.workouts.length === 0 ? (
              <p className="text-sm opacity-50 text-center py-4">Aucun entraînement créé</p>
            ) : (
              <div className="space-y-2">
          ...
          >
            <div className="flex items-center justify-between">
              <span className="font-bold">📚 Bibliothèque d'exercices physiques</span>
        ...
        <div className="bg-white/10 rounded-xl p-4">
          ...
          <div className="mb-4">
                ...
                >
                  ...
                  {data.mindExercises.map(exercise => (
                    <option key={exercise.id} value={exercise.id} className="bg-gray-800 text-white">
                      {exercise.name} ({exercise.type === 'reps' ? 'Répétitions' : 'Chronomètre'})
                    </option>
                  ))}
                </select>
              </div>
            ...
            {!data.mindHabits || data.mindHabits.length === 0 ? (
              <p className="text-sm opacity-50 text-center py-4">Aucune habitude créée</p>
            ) : (
              data.mindHabits.map(habit => (
                <div key={habit.id} className="bg-white/5 p-3 rounded-lg mb-2">
                  ...
                  {habit.target && (
                    <p className="text-xs text-gray-400 mt-1">
                      🎯 {habit.type === 'reps' ? `${habit.target} reps` : `${habit.target} min`}
                    </p>
                  )}
                </div>
              ))
          ...
          >
            <div className="flex items-center justify-between">
              <span className="font-bold">📚 Bibliothèque d'exercices Esprit</span>
  ...
  const renderLibrary = () => {
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">📚 Exercices Physiques</h1>
          <button 
        ...
        {!data.exercises || data.exercises.length === 0 ? (
          <div className="bg-white/10 rounded-xl p-8 text-center">
            <p className="opacity-75 mb-4">Aucun exercice créé</p>
          </div>
        ) : (
          <div className="space-y-3">
              ...
              <div key={exercise.id} className="bg-white/10 rounded-xl p-4">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    ...
                    <p className="text-sm opacity-75">{exercise.category} • {exercise.type}</p>
                  </div>
                  <div className="flex gap-2">
                    <button
                ...
                {exercise.type === 'timing' && exercise.duration && (
                  <p className="text-sm opacity-75">Durée : {formatTime(exercise.duration)}</p>
                )}
              </div>
            ))}
          </div>
  ...
  const renderMindLibrary = () => {
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">📚 Exercices Esprit</h1>
          <button 
            onClick={() => setCurrentView('settings')}
            className="cursor-pointer hover:opacity-75 transition-opacity"
          >
              ...
              >
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="font-bold">{exercise.name}</h3>
                    <p className="text-sm opacity-75">
                      {exercise.type === 'reps' ? '🔢 Répétitions' : '⏱️ Chronomètre'}
                    </p>
                    {exercise.target && (
                      <p className="text-sm text-gray-400 mt-1">
                        🎯 Objectif : {exercise.type === 'reps' 
  ...
  const renderWorkoutEditor = () => {
    const workout = getWorkoutById(editingWorkout);
    if (!workout) return <div>Entraînement introuvable</div>;
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <h1 className="text-xl font-bold">Éditer</h1>
          <button 
            onClick={() => {
              setWorkoutToDelete(workout);
              setShowDeleteWorkoutConfirm(true);
        ...
        <div className="bg-white/10 rounded-xl p-4 space-y-4">
          ...
          >
            <span className="font-semibold">🎨 Personnaliser (icône & couleur)</span>
          ...
          {showWorkoutCustomization && (
            <>
              <div>
                <label className="text-sm opacity-75 block mb-2">Icône</label>
        ...
        <div className="bg-white/10 rounded-xl p-4 space-y-4">
          ...
          <div>
            ...
            >
              <option value="cycle" className="bg-gray-800 text-white">🔄 Cycle rotatif</option>
              <option value="interval" className="bg-gray-800 text-white">⏱️ Récurrence (tous les X jours)</option>
              <option value="weekdays" className="bg-gray-800 text-white">📆 Jours de la semaine</option>
            </select>
          </div>
          ...
          {workout.frequency?.type === 'weekdays' && (
            <div>
              ...
              <p className="text-xs opacity-75 mt-2">
                {workout.frequency?.weekdays?.length === 0 && 'Sélectionne au moins un jour'}
                {workout.frequency?.weekdays?.length === 1 && '1 jour sélectionné'}
                {workout.frequency?.weekdays?.length > 1 && `${workout.frequency.weekdays.length} jours sélectionnés`}
              </p>
            </div>
          )}
          ...
          {workout.frequency?.type === 'cycle' && (
            <p className="text-sm opacity-75">
              Cet entraînement fait partie du cycle rotatif. Il apparaîtra dans l'ordre défini dans les réglages.
            </p>
          )}
        </div>
        ...
        <div className="bg-white/10 rounded-xl p-4">
          ...
          {workout.exercises.length === 0 ? (
            <p className="text-sm opacity-50 text-center py-4">Aucun exercice ajouté</p>
          ) : (
            <div className="space-y-2">
            ...
            {!data.exercises || data.exercises.length === 0 ? (
              ...
              <>
                ...
                <div className="space-y-2 mb-3">
                  ...
                  >
                    <option value="all" className="bg-gray-800 text-white">📂 Toutes les catégories</option>
                    <option value="Renforcement" className="bg-gray-800 text-white">💪 Renforcement</option>
                    <option value="Cardio" className="bg-gray-800 text-white">❤️ Cardio</option>
                    <option value="Mobilité" className="bg-gray-800 text-white">🤸 Mobilité</option>
    ...
    return (
      <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
        <div className="bg-gray-900 rounded-xl p-6 max-w-md w-full space-y-4">
          ...
          <div>
            <label className="text-sm opacity-75 block mb-2">Catégorie</label>
            <select
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-white/10 rounded-lg p-3 text-white"
            >
              <option value="reps" className="bg-gray-800 text-white">Répétitions</option>
              <option value="chrono" className="bg-gray-800 text-white">Chronomètre</option>
          ...
          {exerciseFormData.type === 'timing' && (
            <div>
              <label className="text-sm opacity-75 block mb-2">Durée (secondes)</label>
              <input
                type="number"
        ...
        <div className="bg-gray-900 rounded-xl p-6 max-w-md w-full space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-white/10 rounded-lg p-3"
              placeholder="Ex: Méditation, Respiration..."
            />
          </div>
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-white/10 rounded-lg p-3 text-white"
            >
              <option value="chrono" className="bg-gray-800 text-white">⏱️ Chronomètre (temps libre)</option>
              <option value="reps" className="bg-gray-800 text-white">🔢 Répétitions (nombre)</option>
            </select>
            <p className="text-xs opacity-75 mt-2">
              {mindExerciseFormData.type === 'chrono' 
                ? 'Le timer part de 0 et monte. Tu arrêtes quand tu veux.' 
          ...
          <div>
            <label className="text-sm opacity-75 block mb-2">
              Objectif {mindExerciseFormData.type === 'chrono' ? '(temps en minutes)' : '(nombre de répétitions)'}
            </label>
            <input
              type="number"
            ...
            <p className="text-xs opacity-75 mt-2">
              {mindExerciseFormData.type === 'chrono' 
                ? 'Durée visée en minutes (optionnel)' 
        ...
        <div className="bg-gray-900 rounded-xl p-6 max-w-md w-full space-y-4">
          ...
          {availableWorkouts.length === 0 ? (
            <p className="text-sm opacity-75 text-center py-4">
              Tous les entraînements sont déjà dans le cycle
            </p>
          ) : (
            <div className="space-y-2">
    ...
    return (
      <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
        <div className="bg-gray-900 rounded-xl p-6 max-w-md w-full space-y-4">
          ...
          <div>
            <label className="text-sm opacity-75 block mb-2">Repos après cet exercice (secondes)</label>
            <input
              type="number"
      ...
      <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
        <div className="bg-gray-900 rounded-xl p-6 max-w-md w-full space-y-4 border-2 border-red-500">
          <div className="text-center">
            ...
            <h2 className="text-2xl font-bold mb-2">Supprimer l'entraînement</h2>
            <p className="text-sm opacity-75 mb-4">
          ...
          <div className="bg-red-500/10 rounded-lg p-4 text-sm">
            <p className="text-red-400">
              ⚠️ Cet entraînement sera également retiré de votre cycle.
            </p>
          </div>
      ...
      <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
        <div className="bg-gray-900 rounded-xl p-6 max-w-md w-full space-y-4 border-2 border-red-500">
          ...
          <div className="bg-white/10 rounded-lg p-4">
            ...
            <p className="text-sm text-center opacity-75 mt-2">
              {habitToDelete.type === 'reps' ? '🔢 Répétitions' : '⏱️ Chronomètre'}
            </p>
          </div>
        ...
        <div className="bg-gray-900 rounded-xl p-6 max-w-md w-full space-y-4">
          ...
          {sourceExercise && (
            <div className="bg-blue-500/20 border border-blue-500/50 rounded-lg p-3">
              <p className="text-sm text-blue-300">
                🔗 Lié à l'exercice "{sourceExercise.name}"
              </p>
              <p className="text-xs opacity-75 mt-1">
                Les modifications s'appliqueront aussi à l'exercice dans la bibliothèque
              </p>
            </div>
          )}
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-white/10 rounded-lg p-3 text-white"
            >
              <option value="chrono" className="bg-gray-800 text-white">⏱️ Chronomètre</option>
              <option value="reps" className="bg-gray-800 text-white">🔢 Répétitions</option>
            </select>
          </div>
          ...
          <div>
            <label className="text-sm opacity-75 block mb-2">
              Objectif {editingHabit.type === 'chrono' ? '(minutes)' : '(répétitions)'}
            </label>
            <input
              type="number"
            ...
            <button
              onClick={() => {
                ...
                setData(prev => {
                  ...
                  // Si l'habitude a un exercice source, le mettre à jour aussi
    ...
    return (
      <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
        <div className="bg-gray-900 rounded-xl p-6 max-w-md w-full space-y-4 border-2 border-red-500">
          <div className="text-center">
            <div className="text-6xl mb-4">🗑️</div>
            <h2 className="text-2xl font-bold mb-2">Supprimer la séance</h2>
            <p className="text-sm opacity-75 mb-4">
              Voulez-vous vraiment supprimer cette séance de l'historique ?
            </p>
          </div>
          ...
          <div className="bg-white/10 rounded-lg p-4">
            ...
            <p className="text-sm text-center opacity-75 mt-2">
              {historyEntryToDelete.type === 'body' ? '💪 Corps' : '🧠 Esprit'} • {new Date(historyEntryToDelete.date).toLocaleDateString('fr-FR')}
            </p>
            {historyEntryToDelete.duration && (
              <p className="text-sm text-center opacity-75 mt-1">
                ⏱️ {formatTime(historyEntryToDelete.duration)}
              </p>
            )}
          </div>
            ...
            <button
              onClick={() => {
                ...
                setData(prev => {
                  ...
                  // Si c'était une séance d'aujourd'hui, réinitialiser lastCompletedDate
                  if (wasToday) {
                    if (entry.type === 'body') {
                      // Chercher la dernière séance de ce workout avant celle-ci
                      const previousWorkoutSession = prev.history
                        .filter(h => h.type === 'body' && h.workoutId === entry.workoutId && h.id !== entry.id)
                        .sort((a, b) => new Date(b.date) - new Date(a.date))[0];
                      
                      // Mettre à jour le workout concerné
                      ...
                      // Si c'était un workout du cycle, mettre à jour bodyCycle aussi
                      ...
                      // Chercher la dernière séance de cette habitude avant celle-ci
    ...
    return (
      <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
        <div className="bg-gray-900 rounded-xl p-6 max-w-md w-full space-y-4 border-2 border-red-500">
          <div className="text-center">
            <div className="text-6xl mb-4">⚠️</div>
            <h2 className="text-2xl font-bold mb-2">Abandonner l'entraînement ?</h2>
            <p className="text-sm opacity-75 mb-4">
      ...
      <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
        <div className="bg-gray-900 rounded-xl p-6 max-w-md w-full space-y-4 border-2 border-red-500">
          <div className="text-center">
            ...
            <p className="text-sm opacity-75 mb-4">
              Cet élément sera retiré de ton cycle d'entraînement
            </p>
          </div>
          
          <div className="bg-white/10 rounded-lg p-4">
            <p className="font-bold text-center text-lg">
              {isWorkout ? (workout?.name || 'Entraînement inconnu') : `☕ ${step.name}`}
            </p>
